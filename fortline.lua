--[[
    ...
    ├── Silent Aim (modifikasi WeaponFired arah tembak)
    ├── Rapid Fire (spam WeaponActivated)
    ├── Auto Reload (WeaponReloadRequest otomatis)
    ├── No Reload (cancel reload langsung)
    ├── ESP (box/name/hp/distance)
    ├── Kill Feed Monitor (SquadKillFeed)
    ├── Auto Respawn (RespawnRequest)
    └── XP Boost (XPBoostEvent)
    ============================================================================
]]
...
local WeaponNet=RS:FindFirstChild("WeaponsSystem") and RS.WeaponsSystem:FindFirstChild("Network")
...
R.GetTopKills       = GetR(Events,"GetTopKillsFunction")
...
R.JumpPad           = GetR(Events,"JumpPadEvent")
R.SquadInvite       = GetR(Events,"SquadInvite")
R.SquadResponse     = GetR(Events,"SquadResponse")
R.SquadKillFeed     = GetR(Events,"SquadKillFeed")
R.BatchedGameEvents = GetR(Events,"BatchedGameEvents")
...
local Config={
    ...
    Triggerbot={
        Enabled=false,
        Threshold=30,   -- pixel dari crosshair
        Delay=0.05,
    },
    -- Silent Aim
    SilentAim={
...
local function ShowToast(msg,isOn)
    pcall(function()
        ...
        Instance.new("UIStroke",f).Color=isOn and Color3.fromRGB(40,200,60) or Color3.fromRGB(200,50,50)
...
local function CheckLOS(tPos,tChar)
    local _fs={LocalPlayer.Character,tChar}
...
-- [Silent Aim] Hook WeaponFired untuk modifikasi arah tembak
...
local function InitSilentAim()
    ...
    pcall(function()
        ...
        mt.__namecall=newcclosure(function(self,...)
            ...
            -- Hook WeaponFired — modifikasi arah tembak ke target
            if Config.SilentAim.Enabled and _currentTarget then
                if method=="FireServer" then
                    local args={...}
                    -- Cek apakah ini WeaponFired atau WeaponHit
                    local selfName=pcall(function() return self:GetFullName() end)
                        and self:GetFullName() or ""
                    if selfName:find("WeaponFired") or selfName:find("WeaponHit") then
                        -- Inject posisi kepala target ke argumen
                        local targetPos=_currentTarget.part.Position
                        -- Coba replace argumen Vector3/CFrame pertama
                        for i,arg in ipairs(args) do
                            if typeof(arg)=="Vector3" then
                                args[i]=targetPeif typeof(arg)=="CFrame" then
                                args[i]=CFrame.new(targetPos); break
                            elseif typeof(arg)=="RaycastResult" then
                                break
                            end
                        end
...
Core:Add(RunService.RenderStepped:Connect(function()
    ...
    -- Standard Aimbot — kunci kamera ke kepala
    ...
    if Config.Triggerbot.Enabled and result then
        ...
        if now-_lastTrig>=Config.Triggerbot.Delay then
            ...
            if dist<Config.Triggerbot.Threshold then
                -- Fire WeaponActivated (lebih efektif dari mouse1click di FPS)
                pcall(function()
                    if R.WeaponActivated then
                        R.WeaponActivated:FireServer()
                    end
                end)
...
local function StartRapidFire()
    ...
    _rapidConn=RunService.Heartbeat:Connect(function(dt)
        ...
        if not _hasTarget then return end -- Hanya fire saat ada target
        pcall(function()
...
-- Listen WeaponReloaded untuk deteksi kapan reload
local _autoReloadConn
local function StartAutoReload()
    if _autoReloadConn then _autoReloadConn:Disconnect(); _autoReloadConn=nil end

    -- Monitor saat senjata perlu reload
    -- Deteksi dari WeaponFired event (saat ammo 0 → auto reload)
    if R.WeaponReloaded then
        _autoReloadConn=R.WeaponReloaded.OnClientEvent:Connect(function()
            if not Config.Weapon.AutoReload then return end
            -- Reload selesai, siap tembak lagi
        end)
    end

    -- Loop check ammo via tool attributes
    local reloadConn=RunService.Heartbeat:Connect(function()
        if not Config.Weapon.AutoReload then return end
...
Core:Add(function() if _autoReloadConn then _autoReloadConn:Disconnect() end end)

-- No Reload — cancel reload saat dimulai
local _noReloadConn
local function StartNoReload()
    if _noReloadConn then _noReloadConn:Disconnect(); _noReloadConn=nil end
    if not R.WeaponReloadCancel then return end
    -- Hook: saat reload dimulai, langsung cancel
    _noReloadConn=RunService.Heartbeat:Connect(function()
        ...
        pcall(function() R.WeaponReloadCancel:FireServer() end)
    end)
end
...
local function CreateESP(player)
    ...
    local txt=Drawing.new("Text"); txt.Size=12; txt.Center=true; txt.Outline=true
    txt.Color=ec; txt.Visible=false; txt.ZIndex=5; c.Text=txt

    local hpBg=Drawing.new("Line"); hpBg.Thickness=3; hpBg.Color=Color3.new(0,0,0)
    hpBg.Visible=false; c.HpBg=hpBg
    local hpFg=Drawing.new("Line"); hpFg.Thickness=1.8; hpFg.Visible=false; c.HpFg=hpFg

    local dTxt=Drawing.new("Text"); dTxt.Size=10; dTxt.Center=true; dTxt.Outline=true
    dTxt.Color=Color3.fromRGB(255,230,80); dTxt.Visible=false; dTxt.ZIndex=5; c.DistText=dTxt

    local snap=Drawing.new("Line"); snap.Color=ec; snap.Thickness=1
    snap.Visible=false; snap.ZIndex=3; c.SnapLine=snap

    -- Highlight tembus dinding
...
Core:Add(RunService.RenderStepped:Connect(function()
    ...
    for player,c in pairs(ESPCache) do
        ...
        if Config.ESP.ShowName then
            ...
            c.Text.Text=player.Name..(weaponName~="" and " ["..weaponName.."]" or "")
...
local function ShowKillFeed(msg,color)
    pcall(function()
        if not _killFeedGUI or not _killFeedGUI.Parent then
            local sg=Instance.new("ScreenGui",SafeGUI)
        ...
        local entry=Instance.new("Frame",_killFeedGUI)
        ...
        task.delay(4,function()
            for i=1,10 do
                pcall(function() entry.BackgroundTransparency=0.4+(i*0.06); l.TextTransparency=i/10 end)
                task.wait(0.05)
            end
...
if R.SquadKillFeed then
    Core:Add(R.SquadKillFeed.OnClientEvent:Connect(function(data)
        ...
        if type(data)=="table" then
            local killer=data.Killer or data.killer or ""
            local victim=data.Victim or data.victim or ""
            local weapon=data.Weapon or data.weapon or ""
...
-- Listen revenge event
if R.Revenge then
    Core:Add(R.Revenge.OnClientEvent:Connect(function(data)
        ShowKillFeed("⚔️ REVENGE kesempatan!",Color3.fromRGB(255,100,100))
    end))
end
...
_fpsDraw.Size=13; _fpsDraw.Center=true; _fpsDraw.Outline=true
_fpsDraw.Visible=true; _fpsDraw.ZIndex=11
...
function UI:Build()
    ...
    TopBar.Size=UDim2.new(1,0,0,36); TopBar.BackgroundColor3=Color3.fromRGB(20,8,8)
    ...
    TL.Text="🎯  NEXUS  FORTLINE"; TL.TextColor3=Color3.fromRGB(255,80,80)
    ...
    Instance.new("UIStroke",Pill).Color=Color3.fromRGB(255,60,60)
    self.Pill=Pill
...
function UI:AddTab(name)
    ...
    btn.Size=UDim2.new(0,42,0,20); btn.BackgroundColor3=Color3.fromRGB(28,10,10)
    ...
    local function activate()
        for _,t in ipairs(self._tabPages) do
            t.page.Visible=false; t.btn.BackgroundColor3=Color3.fromRGB(28,10,10)
            t.btn.TextColor3=Color3.fromRGB(180,80,80)
        end
        page.Visible=true; btn.BackgroundColor3=Color3.fromRGB(200,40,40)
...
function UI:Section(parent,text)
    ...
    l.Text="── "..text.." ──"; l.TextColor3=Color3.fromRGB(255,80,80)
...
function UI:Toggle(parent,label,callback,col)
    ...
    card.Size=UDim2.new(1,0,0,26); card.BackgroundColor3=Color3.fromRGB(22,10,10); card.BorderSizePixel=0
    Instance.new("UICorner",card).CornerRadius=UDim.new(0,6)
    local lbl=Instance.new("TextLabel",card)
    lbl.Size=UDim2.new(1,-48,1,0); lbl.Position=UDim2.new(0,9,0,0); lbl.BackgroundTransparency=1
    lbl.Text=label; lbl.TextColor3=Color3.fromRGB(235,200,200)
    ...
    pill.BackgroundColor3=Color3.fromRGB(50,20,20); pill.Text=""; pill.BorderSizePixel=0
    ...
    Core:Add(pill.MouseButton1Click:Connect(function()
        ...
        if state then
            ...
            pill.BackgroundColor3=Color3.fromRGB(50,20,20); knob.Position=UDim2.new(0,2,0.5,-5)
...
function UI:MakeSlider(parent,labelText,initVal,minVal,maxVal,onChange)
    ...
    fc.Size=UDim2.new(1,0,0,42); fc.BackgroundColor3=Color3.fromRGB(22,10,10); fc.BorderSizePixel=0
    ...
    fl.TextColor3=Color3.fromRGB(220,180,180); fl.Font=Enum.Font.GothamSemibold; fl.TextSize=11; fl.TextXAlignment=Enum.TextXAlignment.Left
    ...
    fi.Size=UDim2.new(ratio,0,1,0); fi.BackgroundColor3=Color3.fromRGB(200,40,40); fi.BorderSizePixel=0
...
do
    ...
    UI:Toggle(p,"Silent Aim (WeaponFired)",function(v)
        Config.SilentAim.Enabled=v
    end,Color3.fromRGB(255,120,40))
    UI:Toggle(p,"Triggerbot (Auto Tembak)",function(v)
    ...
    UI:MakeSlider(p,"Trigger Threshold (px)",Config.Triggerbot.Threshold,5,100,function(v)
...
do
    ...
    UI:MakeSlider(p,"Rapid Fire Delay (ms)",math.floor(Config.Weapon.RapidDelay*1000),10,500,function(v)
...
do
    ...
    sCard.Size=UDim2.new(1,0,0,30); sCard.BackgroundColor3=Color3.fromRGB(14,20,14); sCard.BorderSizePixel=0
    Instance.new("UICorner",sCard).CornerRadius=UDim.new(0,6)
    Instance.new("UIStroke",sCard).Color=Color3.fromRGB(40,130,40)
    local sL=Instance.new("TextLabel",sCard)
    sL.Size=UDim2.new(1,-14,1,-8); sL.Position=UDim2.new(0,7,0,4); sL.BackgroundTransparency=1
    sL.Text=string.format("✅ Remote loaded: %d | WeaponNet: %s",loaded,WeaponNet and "OK" or "❌")
...
print("✅ NEXUS Fortline Suite — Loaded")
print("🎯 Aimbot + Silent Aim + Triggerbot")
print("🔫 Rapid Fire + Auto Reload")
print("👁 ESP + Kill Feed")
