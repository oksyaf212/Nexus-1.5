--[[
    ============================================================================
    PROJECT   : NEXUS v1.6 — Phenomenon Edition
    PLATFORM  : Delta Executor Android
    AUTHOR    : Claude Sonnet 4.6
    NEW v1.6:
    ├── UI      : Theme (5 preset), Resize, Opacity, Undetected Mode
    ├── QoL     : Server Info, Chat Bypass, Teleport Player, Anti Void, Inf Stamina
    ├── RPG     : Auto Quest, Blink Attack, Auto Collect, Level ESP, Dungeon
    ├── Combat  : Auto Combo, Block Predict, Parry Timing
    ├── Sim     : Auto Click, Auto Rebirth, Multiplier ESP
    ├── FPS     : No Recoil, Rapid Fire, Ammo ESP
    └── World   : World ESP, Safe Zone, Weather Alert
    ============================================================================
]]
...
local Config = {
    ESP = {
        ...
        ShowHeadDot=true,ShowSkeleton=true,ShowSnapLine=true,ShowChams=false,
        ShowLevelTag=true, -- [v1.6] Level tag di atas kepala
        ...
        AdaptiveOpacity=false, -- [v1.6] Opacity adaptif berdasarkan jarak
    },
    Aimbot = {
        ...
        Priority="FOV", -- [v1.6] FOV / LowestHP / Nearest / HighestThreat
        NoRecoil=false, -- [v1.6]
        RapidFire=false,-- [v1.6]
    },
    Mods = {
        ...
        InfStamina=false,  -- [v1.6]
        AntiVoid=false,    -- [v1.6]
        BunnyHop=false,    -- [v1.6]
    },
    Crosshair={Enabled=false,Style="Cross",Color=Color3.fromRGB(255,255,255),HitColor=Color3.fromRGB(255,60,60),Size=10,Thickness=1.5},
...
local function SaveConfig()
    pcall(function()
        local d={
            ESPEnabled=Config.ESP.Enabled,ESPHighlight=Config.ESP.ShowHighlight,
            ESPBox=Config.ESP.ShowBox,ESPName=Config.ESP.ShowName,
            ESPHealth=Config.ESP.ShowHealth,ESPHealthNum=Config.ESP.ShowHealthNum,
            ESPDistance=Config.ESP.ShowDistance,ESPSkeleton=Config.ESP.ShowSkeleton,
            ESPSnapLine=Config.ESP.ShowSnapLine,ESPChams=Config.ESP.ShowChams,
            ESPLevelTag=Config.ESP.ShowLevelTag,ESPAdaptive=Config.ESP.AdaptiveOpacity,
            AimbotEnabled=Config.Aimbot.Enabled,AimbotFOV=Config.Aimbot.FOVRadius,
            AimbotSmooth=Config.Aimbot.Smoothness,AimbotWall=Config.Aimbot.WallCheck,
            AimbotTeam=Config.Aimbot.TeamCheck,AimbotPredict=Config.Aimbot.PredictMovement,
            AimbotSilent=Config.Aimbot.SilentAim,AimbotTrigger=Config.Aimbot.Triggerbot,
            AimbotKillAura=Config.Aimbot.KillAura,AimbotPriority=Config.Aimbot.Priority,
            SpeedTier=Config.Mods.SpeedTier,
            UITheme=Config.UI.Theme,UIOpacity=Config.UI.Opacity,UIScale=Config.UI.Scale,
            CrosshairEnabled=Config.Crosshair.Enabled,CrosshairStyle=Config.Crosshair.Style,
            RadarEnabled=Config.Radar.Enabled,RadarNames=Config.Radar.ShowNames,
            FPSEnabled=Config.FPSCounter.Enabled,SpectatorEnabled=Config.Spectator.Enabled,
        }
...
local function LoadConfig()
    pcall(function()
        ...
        Config.ESP.ShowLevelTag=d.ESPLevelTag~=false
        Config.ESP.AdaptiveOpacity=d.ESPAdaptive or false
        ...
        Config.UI.Scale=d.UIScale or 1.0
...
Core:Add(RunService.RenderStepped:Connect(function()
    ...
    for player,c in pairs(ESPCache) do
        ...
        -- [v1.6] Adaptive opacity berdasarkan jarak
        if Config.ESP.AdaptiveOpacity then
            local alpha=math.clamp(dist/Config.ESP.MaxDistance,0.15,0.85)
            c.Highlight.FillTransparency=alpha
        else
        ...
        -- [v1.6] Level Tag — cari nilai level dari leaderstats
        if Config.ESP.ShowLevelTag then
            local ls=player:FindFirstChild("leaderstats")
            local lvlVal=ls and (ls:FindFirstChild("Level") or ls:FindFirstChild("Lvl") or ls:FindFirstChild("LV"))
            if lvlVal then
                c.LevelTag.Text="Lv."..tostring(lvlVal.Value)
                c.LevelTag.Position=Vector2.new(cx,tV.Y-28); c.LevelTag.Visible=true
...
--[[
    [v1.6] Target Priority System:
    FOV      = paling dekat ke tengah layar (default)
    LowestHP = yang HP-nya paling sedikit
    Nearest  = paling dekat ke karakter kita
    Threat   = yang sedang menyerang kita (jika bisa dideteksi)
]]
local function GetBestTarget()
    ...
    for _,pl in ipairs(Players:GetPlayers()) do
        ...
        -- Hitung score berdasarkan priority
        local score
        if Config.Aimbot.Priority=="FOV" then
            score=screenDist
        elseif Config.Aimbot.Priority=="LowestHP" then
            score=hum.Health -- semakin kecil HP, semakin kecil score, semakin diprioritaskan
        ...
        if score<bestScore then
            bestScore=score
            best={part=head,hrp=hrp,char=char,player=pl}
        end
    end
    return best
end
...
Core:Add(RunService.RenderStepped:Connect(function()
    ...
    if Config.Aimbot.Enabled and result then
        ...
        local tp=PredictPos(result.hrp,result.part)
        if (cp-tp).Magnitude>0.1 then
            Camera.CFrame=Camera.CFrame:Lerp(CFrame.lookAt(cp,tp),Config.Aimbot.Smoothness)
        end
    end
    if Config.Aimbot.SilentAim and result and not Config.Aimbot.Enabled then
        ...
        if (cp-tp).Magnitude>0.1 then
            ...
            Camera.CFrame=CFrame.lookAt(cp,tp)
    ...
    if Config.Aimbot.Triggerbot and result then
        ...
        if now-_lastTrig>=Config.Aimbot.TriggerDelay then
            ...
            if (Vector2.new(sp.X,sp.Y)-ctr).Magnitude<25 then
                pcall(function() mouse1click() end)
            end
        end
    end
    -- [v1.6] No Recoil — kompensasi recoil dengan invert mouse delta
    if Config.Aimbot.NoRecoil then
        pcall(function()
            -- Implementasi passif: setiap frame sedikit stabilkan kamera
...
Core:Add(RunService.Heartbeat:Connect(function()
    ...
    if hrp.Position.Y > -100 then
        _lastSafePos=hrp.CFrame
    elseif _lastSafePos then
        hrp.CFrame=_lastSafePos
        ShowToast("Anti Void aktif!",true)
    end
end))
...
local function StartFly()
    ...
    bv.MaxForce=Vector3.new(math.huge,math.huge,math.huge); bv.Velocity=Vector3.zero; bv.Parent=hrp; _flyBV=bv
    local bav=Instance.new("BodyAngularVelocity")
    bav.MaxTorque=Vector3.new(math.huge,math.huge,math.huge); bav.AngularVelocity=Vector3.zero; bav.Parent=hrp; _flyBAV=bav
    local jt=0
...
local function SetFullBright(on)
    if on then
        ...
        for k,v in pairs(_origL) do pcall(function() Lighting[k]=v end) end
    end
end
...
-- ============================================================================
-- [16] RPG MODULE — Blink Attack System
-- ============================================================================
...
--[[
    BLINK ATTACK:
    Klik 1 → Simpan posisi → Teleport ke monster → Serang
    Klik 2 → Teleport balik ke posisi asal → Serang
    Radius deteksi: Config.RPG.BlinkRadius (default 400m)
]]
local function DoBlinkAttack()
    ...
    if not _blinkState then
        -- Klik 1: cari monster & teleport ke sana
        local target=GetNearestMonster(Config.RPG.BlinkRadius)
        if target then
            local tHRP=target:FindFirstChild("HumanoidRootPart")
            if tHRP then
                _blinkOrigin=hrp.CFrame  -- simpan posisi asal
                -- Teleport ke depan monster
                hrp.CFrame=tHRP.CFrame*CFrame.new(0,0,-4)
                task.wait(0.05)
                -- Serang monster
                pcall(function() mouse1click() end)
                _blinkState=true
                ShowToast("Blink → Monster!",true)
            end
        else
            ShowToast("Tidak ada monster < "..Config.RPG.BlinkRadius.."m",false)
        end
    else
        -- Klik 2: teleport balik ke posisi asal
...
local function StartAutoFarm()
    ...
    _farmConn=RunService.Heartbeat:Connect(function(dt)
        ...
        t=t+dt; if t<0.2 then return end; t=0
        task.spawn(function()
            local target=GetNearestMonster(500)
            if target then
                local tHRP=target:FindFirstChild("HumanoidRootPart")
                local myHRP=LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    ...
    _collectConn=RunService.Heartbeat:Connect(function(dt)
        ...
        for _,obj in ipairs(Workspace:GetDescendants()) do
            if obj:IsA("BasePart") or obj:IsA("MeshPart") then
                ...
                if name:find("drop") or name:find("loot") or name:find("item")
                or name:find("coin") or name:find("gem") or name:find("pickup") then
                    local dist=(obj.Position-myHRP.Position).Magnitude
                    if dist<=Config.RPG.CollectRadius then
        ...
        for _,obj in ipairs(Workspace:GetDescendants()) do
            if obj:IsA("BasePart") then
                ...
                if name:find("dungeon") or name:find("portal") or name:find("gate") or name:find("door") then
                    ...
                    if dist<=10 then
                        pcall(function() firetouchinterest(myHRP,obj,0) end)
                        ShowToast("Dungeon entrance detected!",true)
                    end
                end
            end
        end
...
-- Block Predict — block sebelum kena damage
local _blockPredictConn
local function StartBlockPredict()
    if _blockPredictConn then _blockPredictConn:Disconnect(); _blockPredictConn=nil end
    if not Config.Combat.BlockPredict then return end
    -- Hook pada Humanoid HealthChanged untuk trigger block
    local char=LocalPlayer.Character
    local hum=char and char:FindFirstChildOfClass("Humanoid")
    if not hum then return end
    _blockPredictConn=hum.HealthChanged:Connect(function(health)
        if health<hum.MaxHealth then
            -- Simulasi block input saat kena damage
            pcall(function()
                -- Game-specific: coba berbagai cara
                fireproximityprompt(char)
            end)
        end
    end)
end
...
local function StartParryTiming()
    ...
    _parryConn=hum.HealthChanged:Connect(function(health)
        local prev=hum.MaxHealth
        if health<prev then
            -- Window parry: block tepat saat kena
            task.wait(0.02) -- slight delay untuk parry window
...
local function StartAutoRebirth()
    ...
    _rebirthConn=RunService.Heartbeat:Connect(function(dt)
        ...
        t=t+dt; if t<2 then return end; t=0
        -- Scan GUI untuk tombol rebirth
        local function scanGui(parent)
            for _,obj in ipairs(parent:GetDescendants()) do
                if obj:IsA("TextButton") or obj:IsA("ImageButton") then
                    local name=obj.Name:lower()
                    local text=(obj:IsA("TextButton") and obj.Text:lower()) or ""
                    if name:find("rebirth") or text:find("rebirth")
                    or name:find("prestige") or text:find("prestige") then
                        pcall(function()
                            obj.MouseButton1Click:Fire()
                        end)
                        ShowToast("Auto Rebirth triggered!",true)
                    end
                end
            end
        end
...
-- Multiplier ESP (Drawing text di atas area)
local _multObjs={}
local function UpdateMultiplierESP()
    for _,o in ipairs(_multObjs) do pcall(function() o:Remove() end) end; _multObjs={}
    if not Config.Sim.MultiplierESP then return end
    for _,obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("Model") or obj:IsA("BasePart") then
            ...
            if name:find("zone") or name:find("area") or name:find("mult") then
                ...
                if part then
                    local sp,on=Camera:WorldToViewportPoint(part.Position+Vector3.new(0,5,0))
                    if on then
                        local t=Drawing.new("Text"); t.Size=14; t.Center=true; t.Outline=true
                        t.Color=Color3.fromRGB(255,220,50)
                        t.Text="⚡ "..obj.Name; t.Position=Vector2.new(sp.X,sp.Y)
                        t.Visible=true; t.ZIndex=7
...
-- World ESP — tampilkan objek penting di dunia
local _worldObjs={}
local _worldTimer=0
Core:Add(RunService.RenderStepped:Connect(function(dt)
                    ...
                    if part then
                        ...
                        if dist<2000 then
                            ...
                            if on then
                                ...
                                t.Text=string.format("📍 %s [%.0fm]",obj.Name,dist)
    ...
    for _,obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("BasePart") then
            ...
            if name:find("safe") or name:find("lobby") or name:find("spawn") or name:find("town") then
                ...
                if dist<50 then isSafe=true; break end
            end
        end
    end
    if isSafe~=_inSafeZone then
        _inSafeZone=isSafe
        ShowToast(isSafe and "✅ Masuk Safe Zone" or "⚠️ Keluar Safe Zone",isSafe)
    end
end))

-- Weather Alert
local _lastWeather=""
local _weatherTimer=0
Core:Add(RunService.Heartbeat:Connect(function(dt)
    ...
    local weather=""
    if Lighting.FogEnd<1000 then weather="🌫️ Foggy"
    elseif Lighting.ClockTime<6 or Lighting.ClockTime>20 then weather="🌙 Night"
    elseif Lighting.Brightness>3 then weather="☀️ Bright Day"
    else weather="🌤️ Day" end
    if weather~=_lastWeather then
        _lastWeather=weather
        ShowToast("Weather: "..weather,true)
    end
end))
...
local function TeleportToPlayer(targetName)
    ...
    if not target then
        for _,pl in ipairs(Players:GetPlayers()) do
            if pl.Name:lower():find(targetName:lower()) then target=pl; break end
        end
    end
...
local function SendChat(msg)
    if not Config.Chat.Bypass then
        pcall(function() game:GetService("ReplicatedStorage"):FindFirstChild("DefaultChatSystemChatEvents")
            :FindFirstChild("SayMessageRequest"):FireServer(msg,"All") end)
    else
        -- Bypass dengan karakter unicode
        local bypassed=""
        for i=1,#msg do
            local c=msg:sub(i,i)
            bypassed=bypassed..c..string.char(8203) -- zero-width space
        end
        pcall(function() game:GetService("ReplicatedStorage"):FindFirstChild("DefaultChatSystemChatEvents")
            :FindFirstChild("SayMessageRequest"):FireServer(bypassed,"All") end)
    end
end
...
-- Undetected Mode — sembunyikan semua Drawing & GUI
local _undetectedActive=false
local function SetUndetectedMode(on)
    _undetectedActive=on
    -- Sembunyikan/tampilkan semua Drawing objects
    ...
    _fpsDraw.Visible=not on
    FOVCircle.Visible=not on
    -- Sembunyikan/tampilkan GUI utama
    local nexusUI=SafeGUI:FindFirstChild("Nexus_UI")
    if nexusUI then nexusUI.Enabled=not on end
    ShowToast(on and "Undetected Mode ON" or "Undetected Mode OFF",not on)
end

-- ============================================================================
-- [21] UI MODULE — dengan Theme System
-- ============================================================================
local UI={_tabPages={},_allCards={},_stroke=nil}
...
local function ApplyTheme(themeName)
    ...
    -- Update semua card yang sudah dibuat
    for _,card in ipairs(UI._allCards) do
        pcall(function()
            if card.type=="wrapper_stroke" then card.obj.Color=theme.primary end
            if card.type=="topbar" then card.obj.BackgroundColor3=theme.topbar end
            if card.type=="tab_active" then card.obj.BackgroundColor3=theme.primary end
            if card.type=="scroll_bar" then card.obj.ScrollBarImageColor3=theme.primary end
        end)
    end
    -- Update radar border
...
function UI:Build()
    ...
    local baseW=262; local baseH=450
    Wrapper.Size=UDim2.new(0,baseW,0,baseH)
    Wrapper.Position=UDim2.new(0.04,0,0.06,0)
    Wrapper.BackgroundTransparency=1
    Instance.new("UICorner",Wrapper).CornerRadius=UDim.new(0,12)
    local WStroke=Instance.new("UIStroke",Wrapper)
    WStroke.Color=CurrentTheme.primary; WStroke.Thickness=1.5
    table.insert(self._allCards,{type="wrapper_stroke",obj=WStroke})
    self.Wrapper=Wrapper

    local Main=Instance.new("Frame",Wrapper)
    Main.Name="Main"; Main.Size=UDim2.new(1,0,1,0)
    Main.BackgroundColor3=CurrentTheme.bg
    Main.BackgroundTransparency=1-Config.UI.Opacity
    ...
    table.insert(self._allCards,{type="topbar",obj=TopBar})

    local TL=Instance.new("TextLabel",TopBar)
    TL.Size=UDim2.new(1,-96,1,0); TL.Position=UDim2.new(0,11,0,0)
    TL.BackgroundTransparency=1; TL.Text="⚡  NEXUS  v1.6"
    ...
    Content.Name="Content"; Content.Size=UDim2.new(1,0,1,-64)
    Content.Position=UDim2.new(0,0,0,64); Content.BackgroundTransparency=1
...
function UI:AddTab(name)
    ...
    table.insert(self._allCards,{type="scroll_bar",obj=page})
    ...
    btn.Size=UDim2.new(0,36,0,20); btn.BackgroundColor3=Color3.fromRGB(28,28,40)
    btn.Text=name; btn.TextColor3=Color3.fromRGB(150,150,170)
    btn.Font=Enum.Font.GothamSemibold; btn.TextSize=8; btn.BorderSizePixel=0
...
function UI:Toggle(parent,label,callback,col)
    local color=col or CurrentTheme.primary; local state=false
    ...
    lbl.TextColor3=Color3.fromRGB(210,210,215); lbl.Font=Enum.Font.GothamSemibold
    ...
    Core:Add(pill.MouseButton1Click:Connect(function()
        ...
        ShowToast(label,state); SaveConfig(); pcall(callback,state)
    end))
end
...
function UI:HoldButton(parent,label,onH,onR,col)
    ...
    Core:Add(btn.InputBegan:Connect(function(i)
        ...
        or i.UserInputType==Enum.UserInputType.MouseButton1 then pcall(onH) end
    end))
    Core:Add(btn.InputEnded:Connect(function(i)
        if i.UserInputType==Enum.UserInputType.Touch
        or i.UserInputType==Enum.UserInputType.MouseButton1 then pcall(onR) end
    end))
end
...
do
    ...
    UI:Toggle(p,"Level Tag (leaderstats)",function(v) Config.ESP.ShowLevelTag=v end,Color3.fromRGB(180,180,255))
    task.defer(r)
end
...
do
    ...
    UI:Toggle(p,"Rapid Fire",function(v) Config.Aimbot.RapidFire=v end,Color3.fromRGB(255,200,50))
    UI:Section(p,"PRIORITY")
    UI:ChoiceRow(p,"Target Priority",{"FOV","LowestHP","Nearest"},"FOV",function(v)
        Config.Aimbot.Priority=v
    end)
    UI:Section(p,"FILTER")
    ...
    fl.Text="FOV: "..Config.Aimbot.FOVRadius
    ...
    fi.Size=UDim2.new(Config.Aimbot.FOVRadius/500,0,1,0)
    fi.BackgroundColor3=CurrentTheme.primary; fi.BorderSizePixel=0
    ...
    kn.Position=UDim2.new(Config.Aimbot.FOVRadius/500,0,0.5,0)
    ...
    Core:Add(kn.InputBegan:Connect(function(i)
        if i.UserInputType==Enum.UserInputType.Touch or i.UserInputType==Enum.UserInputType.MouseButton1 then ds2=true end
    end))
    Core:Add(UserInputService.InputChanged:Connect(function(i)
        ...
        if i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch then
            ...
            local val=math.clamp(math.floor(rx*500),30,500)
    ...
    Core:Add(UserInputService.InputEnded:Connect(function(i)
        if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then ds2=false; SaveConfig() end
    end))
    task.defer(r)
end
...
do
    ...
    UI:Toggle(p,"Infinite Stamina",function(v) Config.Mods.InfStamina=v end,Color3.fromRGB(100,200,255))
...
do
    ...
    UI:Toggle(p,"Auto Farm + Teleport",function(v)
        Config.RPG.AutoFarm=v; if v then StartAutoFarm() end
    end,Color3.fromRGB(255,80,80))
    UI:Toggle(p,"Auto Quest",function(v)
        Config.RPG.AutoQuest=v; if v then StartAutoQuest() end
    end,Color3.fromRGB(80,200,80))
    UI:Toggle(p,"Auto Collect Loot",function(v)
        Config.RPG.AutoCollect=v; if v then StartAutoCollect() end
    end,Color3.fromRGB(255,200,50))
    UI:Toggle(p,"Dungeon Helper",function(v)
        Config.RPG.DungeonHelper=v; if v then StartDungeonHelper() end
    end,Color3.fromRGB(200,100,255))
    UI:Section(p,"⚡ BLINK ATTACK")
    -- Info card
    local infoCard=Instance.new("Frame",p)
    infoCard.Size=UDim2.new(1,0,0,52); infoCard.BackgroundColor3=Color3.fromRGB(16,22,16); infoCard.BorderSizePixel=0
    Instance.new("UICorner",infoCard).CornerRadius=UDim.new(0,6)
    local iS=Instance.new("UIStroke",infoCard); iS.Color=Color3.fromRGB(40,150,40); iS.Thickness=1
    ...
    iL.Text="Klik 1 → Teleport ke monster → Serang\nKlik 2 → Teleport balik → Serang"
    ...
    UI:Toggle(p,"Aktifkan Blink Attack",function(v) Config.RPG.BlinkAttack=v end,Color3.fromRGB(0,200,255))
    UI:ActionBtn(p,"⚡  BLINK!",Color3.fromRGB(0,160,220),function()
        DoBlinkAttack()
    end)
    task.defer(r)
end
...
do
    ...
    UI:Section(p,"AUTO COMBAT")
    UI:Toggle(p,"Auto Combo",function(v)
        Config.Combat.AutoCombo=v; if v then StartAutoCombo() end
    end,Color3.fromRGB(255,80,80))
    UI:Toggle(p,"Block Predict",function(v)
        Config.Combat.BlockPredict=v; StartBlockPredict()
    end,Color3.fromRGB(40,160,255))
    UI:Toggle(p,"Parry Timing",function(v)
        Config.Combat.ParryTiming=v; StartParryTiming()
    end,Color3.fromRGB(255,200,40))
    UI:Section(p,"SIMULATOR")
    UI:Toggle(p,"Auto Click",function(v)
        Config.Sim.AutoClick=v; if v then StartAutoClick() end
    end,Color3.fromRGB(255,120,40))
    UI:Toggle(p,"Auto Rebirth",function(v)
        Config.Sim.AutoRebirth=v; if v then StartAutoRebirth() end
...
do
    ...
    UI:Toggle(p,"World ESP (Chest/Boss/etc)",function(v) Config.World.WorldESP=v end,Color3.fromRGB(255,220,50))
    UI:Toggle(p,"Safe Zone Detector",function(v) Config.World.SafeZoneDetect=v end,Color3.fromRGB(40,200,100))
    UI:Toggle(p,"Weather Alert",function(v) Config.World.WeatherAlert=v end,Color3.fromRGB(100,180,255))
    UI:Section(p,"TELEPORT")
    UI:InputRow(p,"Nama player...","Teleport",CurrentTheme.primary,function(name)
        TeleportToPlayer(name)
    end)
    UI:Section(p,"CHAT")
    UI:Toggle(p,"Chat Bypass",function(v) Config.Chat.Bypass=v end,Color3.fromRGB(200,160,255))
    UI:InputRow(p,"Tulis pesan...","Kirim",Color3.fromRGB(60,120,60),function(msg)
        SendChat(msg)
    end)
    task.defer(r)
end
...
do
    ...
    UI:ChoiceRow(p,"Pilih Tema",{"Blue","Red","Green","Purple","Gold"},"Blue",function(t)
        Config.UI.Theme=t; ApplyTheme(t); SaveConfig()
        ShowToast("Theme: "..t,true)
    end)
    UI:Section(p,"TAMPILAN")
    -- Opacity Slider
    local opCard=Instance.new("Frame",p)
    opCard.Size=UDim2.new(1,0,0,42); opCard.BackgroundColor3=Color3.fromRGB(22,22,33); opCard.BorderSizePixel=0
    Instance.new("UICorner",opCard).CornerRadius=UDim.new(0,6)
    local opLbl=Instance.new("TextLabel",opCard)
    opLbl.Size=UDim2.new(1,-10,0,18); opLbl.Position=UDim2.new(0,9,0,3); opLbl.BackgroundTransparency=1
    opLbl.Text="Opacity GUI: "..string.format("%.0f%%",Config.UI.Opacity*100)
    opLbl.TextColor3=Color3.fromRGB(200,200,210); opLbl.Font=Enum.Font.GothamSemibold
    opLbl.TextSize=11; opLbl.TextXAlignment=Enum.TextXAlignment.Left
    local opTr=Instance.new("Frame",opCard)
    opTr.Size=UDim2.new(1,-18,0,6); opTr.Position=UDim2.new(0,9,0,28)
    opTr.BackgroundColor3=Color3.fromRGB(35,35,52); opTr.BorderSizePixel=0
    Instance.new("UICorner",opTr).CornerRadius=UDim.new(1,0)
    local opFi=Instance.new("Frame",opTr)
    opFi.Size=UDim2.new(Config.UI.Opacity,0,1,0)
    opFi.BackgroundColor3=CurrentTheme.primary; opFi.BorderSizePixel=0
    Instance.new("UICorner",opFi).CornerRadius=UDim.new(1,0)
    local opKn=Instance.new("TextButton",opTr)
    opKn.Size=UDim2.new(0,14,0,14); opKn.AnchorPoint=Vector2.new(0.5,0.5)
    opKn.Position=UDim2.new(Config.UI.Opacity,0,0.5,0)
    opKn.BackgroundColor3=Color3.fromRGB(255,255,255); opKn.Text=""; opKn.BorderSizePixel=0
    Instance.new("UICorner",opKn).CornerRadius=UDim.new(1,0)
    local opDrag=false
    Core:Add(opKn.InputBegan:Connect(function(i)
        if i.UserInputType==Enum.UserInputType.Touch or i.UserInputType==Enum.UserInputType.MouseButton1 then opDrag=true end
    end))
    Core:Add(UserInputService.InputChanged:Connect(function(i)
        if not opDrag then return end
        if i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch then
            local tp=opTr.AbsolutePosition; local ts=opTr.AbsoluteSize
            local rx=math.clamp((i.Position.X-tp.X)/ts.X,0,1)
            local val=math.clamp(rx,0.2,1.0)
            Config.UI.Opacity=val
            opFi.Size=UDim2.new(rx,0,1,0); opKn.Position=UDim2.new(rx,0,0.5,0)
            opLbl.Text="Opacity GUI: "..string.format("%.0f%%",val*100)
            if UI.Main then UI.Main.BackgroundTransparency=1-val end
        end
    end))
    Core:Add(UserInputService.InputEnded:Connect(function(i)
        if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then opDrag=false; SaveConfig() end
    end))
    ...
    UI:ActionBtn(p,"📊  Lihat Info Server",Color3.fromRGB(40,80,150),function()
        local info=GetServerInfo()
        ShowToast(string.format("Players: %d/%d | ID: %s",
            info.players,info.maxPlayers,info.jobId),true)
    end)
...
-- ============================================================================
print("✅ NEXUS v1.6 — Phenomenon Edition Loaded")
print("🆕 Theme, Opacity, UndetectedMode, ServerInfo")
print("🆕 BlinkAttack, AutoFarm, AutoQuest, AutoCollect")
print("🆕 AutoCombo, BlockPredict, ParryTiming")
print("🆕 AutoClick, AutoRebirth, MultiplierESP")
print("🆕 WorldESP, SafeZone, WeatherAlert, ChatBypass")
print("🆕 InfStamina, AntiVoid, BunnyHop, TargetPriority")
print("🔧 Level Tag ESP, Adaptive Opacity, FOV Priority")
-- ============================================================================
