--[[
    ============================================================================
    PROJECT   : NEXUS — Fortline Suite v4 (Rapid Fire Fix)
    GAME      : FORTLINE (Deathmatch·Ranked·No Rules)
    AUTHOR    : Claude Sonnet 4.6
    FIX v4:
    └── Rapid Fire: hanya fire saat target dekat crosshair
        + jitter manusiawi + threshold slider
    ============================================================================
]]
...
local Config={
    ...
    Weapon={
        RapidFire=false,
        RapidDelay=0.18,          -- 180ms antar tembakan (aman)
        RapidFireThreshold=55,     -- hanya fire saat target < 55px dari crosshair
        AutoReload=false,
        NoReload=false,
    },
    ESP={
...
-- [7] FIRE WEAPON
-- ============================================================================
local function FireWeapon(targetPlayer)
    local tool=GetTool(); if not tool then return false end
    local char=LocalPlayer.Character
    ...
    if not(tChar and tHead and tHum and tHRP) then return false end
        ...
        mt.__namecall=newcclosure(function(self,...)
            ...
            if Config.SilentAim.Enabled and _currentTarget then
                if method=="FireServer" then
                    ...
                    if ok and name:find("WeaponFired") then
                        local args={...}
                        if args[2] and type(args[2])=="table" then
                            local c=LocalPlayer.Character
                            local h=c and c:FindFirstChild("HumanoidRootPart")
                            if h then
                                local orig=h.Position+Vector3.new(0,1.5,0)
                                local tPos=_currentTarget.part.Position
                                args[2].dir=(tPos-orig).Unit
                                args[2].origin=orig
                            end
                        end
...
-- ============================================================================
-- [9] RAPID FIRE — [FIX v4] Threshold crosshair + jitter manusiawi
-- ============================================================================
local _rapidConn
local function StartRapidFire()
    ...
    _rapidConn=RunService.Heartbeat:Connect(function(dt)
        ...
        -- Butuh target aktif
        local target=_currentTarget; if not target then return end

        -- [FIX] Cek jarak target dari tengah layar (crosshair)
        local center=Vector2.new(Camera.ViewportSize.X/2,Camera.ViewportSize.Y/2)
        local sp,onScreen=Camera:WorldToViewportPoint(target.part.Position)
        if not onScreen then return end
        local crosshairDist=(Vector2.new(sp.X,sp.Y)-center).Magnitude

        -- Tidak fire jika target terlalu jauh dari crosshair
        if crosshairDist>Config.Weapon.RapidFireThreshold then return end

        -- [FIX] Jitter manusiawi ±8ms agar tidak robot
        local jitter=math.random(-8,8)/1000
        if jitter>0 then task.wait(jitter) end

        FireWeapon(target.player)
    end)
end
Core:Add(function() if _rapidConn then _rapidConn:Disconnect() end end)

-- ============================================================================
-- [10] AUTO RELOAD — IntValue CurrentAmmo
-- ============================================================================
local _reloadConn
local function StartAutoReload()
    ...
    _reloadConn=RunService.Heartbeat:Connect(function()
        ...
        local tool=GetTool(); if not tool then return end
        local ammoValue=tool:FindFirstChild("CurrentAmmo")
        local ammo=ammoValue and ammoValue.Value
        if ammo and ammo<=0 then
            pcall(function()
                if R.WeaponReload then R.WeaponReload:FireServer(tool) end
            end)
            task.wait(0.5)
        end
    end)
...
local function StartNoReload()
    ...
    _noReloadConn=RunService.Heartbeat:Connect(function()
        ...
        local tool=GetTool()
        if tool and R.WeaponReloadCancel then
            pcall(function() R.WeaponReloadCancel:FireServer(tool) end)
        end
    end)
end
...
local function CreateESP(player)
    ...
    local c={}
    local function nL(t)
        local l=Drawing.new("Line"); l.Color=Config.ESP.EnemyColor
    ...
    txt.Outline=true; txt.Color=Config.ESP.EnemyColor
    ...
    local snap=Drawing.new("Line"); snap.Color=Config.ESP.EnemyColor
    ...
    hl.FillColor=Config.ESP.EnemyColor; hl.OutlineColor=Config.ESP.EnemyColor
...
local function ShowKillFeed(msg,color)
    pcall(function()
        if not _killFeedFrame or not _killFeedFrame.Parent then
            local sg=Instance.new("ScreenGui",SafeGUI)
            sg.Name="FortlineKF"; sg.ResetOnSpawn=false; sg.DisplayOrder=998
            Core:Add(sg)
...
if R.SquadKillFeed then
    Core:Add(R.SquadKillFeed.OnClientEvent:Connect(function(data)
        ...
        if type(data)=="table" then
            ...
            msg=string.format("💀 %s → %s",killer,victim)
        end
...
function UI:Build()
    ...
    TL.BackgroundTransparency=1; TL.Text="🎯  NEXUS  FORTLINE v4"
...
do
    ...
    UI:Section(p,"RAPID FIRE")

    -- Info card
    local ic=Instance.new("Frame",p)
    ic.Size=UDim2.new(1,0,0,40); ic.BackgroundColor3=Color3.fromRGB(18,8,8); ic.BorderSizePixel=0
    Instance.new("UICorner",ic).CornerRadius=UDim.new(0,6)
    Instance.new("UIStroke",ic).Color=Color3.fromRGB(200,60,60)
    local il=Instance.new("TextLabel",ic)
    il.Size=UDim2.new(1,-10,1,-6); il.Position=UDim2.new(0,5,0,3); il.BackgroundTransparency=1
    il.Text="🎯 Hanya fire saat target\ndekat crosshair sesuai threshold"
    il.TextColor3=Color3.fromRGB(220,150,150); il.Font=Enum.Font.Gotham
    ...
    UI:MakeSlider(p,"Delay (ms)",math.floor(Config.Weapon.RapidDelay*1000),80,500,function(v)
        Config.Weapon.RapidDelay=v/1000
    end)

    UI:MakeSlider(p,"Threshold (px)",Config.Weapon.RapidFireThreshold,20,200,function(v)
        Config.Weapon.RapidFireThreshold=v
    end)

    -- Realtime indicator
    local indCard=Instance.new("Frame",p)
    indCard.Size=UDim2.new(1,0,0,28); indCard.BackgroundColor3=Color3.fromRGB(14,6,6); indCard.BorderSizePixel=0
    Instance.new("UICorner",indCard).CornerRadius=UDim.new(0,6)
    local indLabel=Instance.new("TextLabel",indCard)
    indLabel.Size=UDim2.new(1,-10,1,-6); indLabel.Position=UDim2.new(0,5,0,3)
    indLabel.BackgroundTransparency=1; indLabel.Text="⬛ Target: —"
    indLabel.TextColor3=Color3.fromRGB(200,150,150); indLabel.Font=Enum.Font.GothamBold
    indLabel.TextSize=10; indLabel.TextXAlignment=Enum.TextXAlignment.Left

    -- Update indikator jarak crosshair realtime
    Core:Add(RunService.RenderStepped:Connect(function()
        pcall(function()
            if not _currentTarget then
                indLabel.Text="⬛ Target: tidak ada"
                indLabel.TextColor3=Color3.fromRGB(150,150,150)
                return
            end
            local center=Vector2.new(Camera.ViewportSize.X/2,Camera.ViewportSize.Y/2)
            local sp,on=Camera:WorldToViewportPoint(_currentTarget.part.Position)
            if not on then
                indLabel.Text="⬛ Target: diluar layar"
                indLabel.TextColor3=Color3.fromRGB(150,150,150)
                return
            end
            local dist=math.floor((Vector2.new(sp.X,sp.Y)-center).Magnitude)
            local inRange=dist<=Config.Weapon.RapidFireThreshold
            indLabel.Text=(inRange and "🟢" or "🔴")
                .." Target: "..dist.."px | Threshold: "
                ..Config.Weapon.RapidFireThreshold.."px"
            indLabel.TextColor3=inRange
    ...
    -- Ammo display
    local ammoCard=Instance.new("Frame",p)
    ammoCard.Size=UDim2.new(1,0,0,28)
    ammoCard.BackgroundColor3=Color3.fromRGB(18,8,8); ammoCard.BorderSizePixel=0
    Instance.new("UICorner",ammoCard).CornerRadius=UDim.new(0,6)
    local ammoLabel=Instance.new("TextLabel",ammoCard)
    ammoLabel.Size=UDim2.new(1,-10,1,-6); ammoLabel.Position=UDim2.new(0,5,0,3)
    ammoLabel.BackgroundTransparency=1; ammoLabel.Text="🔫 Ammo: --"
    ammoLabel.TextColor3=Color3.fromRGB(255,200,150); ammoLabel.Font=Enum.Font.GothamBold
    ammoLabel.TextSize=11; ammoLabel.TextXAlignment=Enum.TextXAlignment.Left
    Core:Add(RunService.Heartbeat:Connect(function()
        pcall(function()
            local tool=GetTool()
            if tool then
                local ammoVal=tool:FindFirstChild("CurrentAmmo")
                if ammoVal then
                    local color=ammoVal.Value<=5
                        and Color3.fromRGB(255,80,80)
                        or  Color3.fromRGB(255,200,150)
                    ammoLabel.Text="🔫 Ammo: "..tostring(ammoVal.Value)
                    ammoLabel.TextColor3=color
                end
            end
        end)
    end))
...
do
    ...
    sCard.Size=UDim2.new(1,0,0,44)
    sCard.BackgroundColor3=Color3.fromRGB(10,18,10); sCard.BorderSizePixel=0
    ...
    sL.Text="✅ Remote: "..loaded.."\n✅ v4: Rapid Fire threshold + jitter"
...
-- ============================================================================
print("✅ NEXUS Fortline v4 — Rapid Fire Fix Loaded")
print("🎯 Rapid Fire: threshold crosshair + jitter manusiawi")
print("📊 Indikator jarak crosshair realtime di Tab Gun")
-- ============================================================================
