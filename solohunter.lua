--[[
    ============================================================================
    PROJECT   : NEXUS — Solo Hunter Suite
    GAME      : Solo Hunter (RPG Dungeon)
    PLATFORM  : Delta Executor Android
    AUTHOR    : Claude Sonnet 4.6
    BERDASARKAN: Hasil spy & analisis mendalam
    FITUR:
    ├── Auto Attack  ([3]=1.0 + human delay + CancelAttacks skip animasi)
    ├── Auto Collect (CollectDrop otomatis)
    ├── Auto Quest   (RequestQuest + TurnInQuest)
    ├── Auto Heal    (UseHeal + UseMana saat HP rendah)
...
local RSInner=RS:FindFirstChild("ReplicatedStorage")
...
R.UseAbility   = GetR(RemoteServices,"AttackService","RF","UseAbility")
...
R.SurrenderQ   = GetR(RemoteServices,"QuestsV2Service","RF","SurrenderQuest")
R.QuestDone    = GetR(RemoteServices,"QuestsV2Service","RE","QuestReadyToTurnIn")
R.ClaimQuest   = GetR(RemoteServices,"QuestsService","RF","ClaimQuest")
...
-- DUNGEON
R.StartDungeon = GetR(RemoteServices,"DungeonService","RF","StartDungeon")
R.QuickPortal  = GetR(RemoteServices,"DungeonService","RF","SpawnQuickPortal")
R.TpLobby      = GetR(RemoteServices,"DungeonService","RF","TeleportToLobby")
R.MobKilled    = GetR(RemoteServices,"DungeonService","RE","MobKilled")
R.DungeonClear = GetR(RemoteServices,"DungeonService","RE","ClearedDungeon")
...
local Config={
    AutoAttack={
        Enabled=false,
        -- [3]=1.0 selalu (full charge dari hasil spy)
        -- Human-like delay: 0.12~0.22 detik acak
        MinDelay=0.12,
        MaxDelay=0.22,
        SkipAnim=true,  -- CancelAttacks untuk skip animasi
        Range=50,       -- radius deteksi monster
    },
    AutoCollect={
        Enabled=false,
        Range=150,
    ...
    AutoHeal={
        Enabled=false,
        HPThreshold=50, -- heal saat HP < 50%
        ManaThreshold=30,
    },
    AutoSell={
        Enabled=false,
        Interval=60,    -- jual setiap 60 detik
    },
    ESP={
        Enabled=false,
        ShowBox=false,
...
local function ShowToast(msg,isOn)
    pcall(function()
        local ex=SafeGUI:FindFirstChild("SHToast")
        if ex then ex:Destroy() end
        local sg=Instance.new("ScreenGui",SafeGUI)
        sg.Name="SHToast"; sg.ResetOnSpawn=false; sg.DisplayOrder=9999
        Core:Add(sg)
...
-- Random human delay
local function HumanWait(min,max)
    local delay=min+(math.random()*(max-min))
    -- tambah jitter kecil
    delay=delay+(math.random(-5,5)/1000)
...
-- ============================================================================
-- [7] AUTO ATTACK
-- Menggunakan temuan spy:
-- UseWeapon:InvokeServer(0, {}, 1.0)
-- [1]=slot, [2]=modifiers, [3]=timing(1.0=full)
-- CancelAttacks untuk skip animasi = DPS lebih tinggi
-- ============================================================================
local _atkConn
local _atkCount=0

local function StartAutoAttack()
    if _atkConn then _atkConn:Disconnect(); _atkConn=nil end
    task.spawn(function()
        while Config.AutoAttack.Enabled do
            ...
            -- [CORE] UseWeapon dengan [3]=1.0 (full charge)
            -- Ini hasil dari analisis spy kita
            local result=Invoke(R.UseWeapon, 0, {}, 1.0)
            _atkCount=_atkCount+1

            -- Skip animasi dengan CancelAttacks (DPS lebih tinggi)
            if Config.AutoAttack.SkipAnim then
                task.wait(0.03)
                Invoke(R.CancelAtk, LocalPlayer)
            end
...
-- ============================================================================
-- [8] AUTO COLLECT
-- Drop cache dari DropCreatedSignal
-- ============================================================================
local _pendingDrops={}

if R.DropCreated then
    Core:Add(R.DropCreated.OnClientEvent:Connect(function(data)
        if data then table.insert(_pendingDrops,data) end
    end))
end

if R.DropRemoved then
    Core:Add(R.DropRemoved.OnClientEvent:Connect(function(dropId)
        for i,d in ipairs(_pendingDrops) do
            if (d.id or d)==dropId then
                table.remove(_pendingDrops,i); break
            end
        end
    end))
end

-- Chest cache dari SpawnChest
local _pendingChests={}
if R.SpawnChest then
    Core:Add(R.SpawnChest.OnClientEvent:Connect(function(data)
        if data then table.insert(_pendingChests,data) end
    end))
end
...
local function StartAutoCollect()
    ...
    _collectConn=RunService.Heartbeat:Connect(function(dt)
        if not Config.AutoCollect.Enabled then
            _collectConn:Disconnect(); _collectConn=nil; return
        end
        t=t+dt; if t<0.3 then return end; t=0
        ...
        task.spawn(function()
            ...
            -- Collect dari cache
            local snapshot={table.unpack(_pendingDrops)}
            for _,drop in ipairs(snapshot) do
                local dropId=drop.id or drop
                -- Teleport ke drop jika ada posisi
                if drop.position or drop.Position then
                    local pos=drop.position or drop.Position
                    if (pos-myHRP.Position).Magnitude<=Config.AutoCollect.Range then
                        myHRP.CFrame=CFrame.new(pos+Vector3.new(0,3,0))
                        task.wait(0.05)
                    end
                end
                Invoke(R.CollectDrop,dropId)
                task.wait(0.05)
            end

            -- Scan workspace untuk drop
            for _,obj in ipairs(Workspace:GetDescendants()) do
                if obj:IsA("BasePart") or obj:IsA("Model") then
                    local name=obj.Name:lower()
                    if name:find("drop") or name:find("loot") or name:find("gem")
                    or name:find("coin") or name:find("orb") or name:find("crystal") then
            ...
            -- Chest cache
            local chestSnap={table.unpack(_pendingChests)}
            for _,chest in ipairs(chestSnap) do
                Invoke(R.OpenChest,chest)
                task.wait(0.1)
            end
        end)
    end)
...
-- Listen quest ready
if R.QuestDone then
    Core:Add(R.QuestDone.OnClientEvent:Connect(function(questId)
        if Config.AutoQuest.Enabled then
            task.spawn(function()
                task.wait(0.5)
                Invoke(R.TurnIn,questId)
                _questCount=_questCount+1
                ShowToast("Quest turned in! #".._questCount,true)
            end)
        end
    end))
end
...
local function StartAutoQuest()
    ...
    _questConn=RunService.Heartbeat:Connect(function(dt)
        if not Config.AutoQuest.Enabled then
            _questConn:Disconnect(); _questConn=nil; return
        end
        ...
        task.spawn(function()
            ...
            if active then
                for _,q in pairs(active) do
                    if q.completed or q.readyToTurnIn then
                        Invoke(R.TurnIn,q.id or q)
                        _questCount=_questCount+1
                        task.wait(0.3)
                    end
                end
                ...
                -- Request quest baru
                Invoke(R.RequestQuest)
                task.wait(0.5)
                Invoke(R.AssignQuests)
            end
        end)
    end)
end
...
local function StartAutoHeal()
    ...
    _healConn=RunService.Heartbeat:Connect(function(dt)
        if not Config.AutoHeal.Enabled then
            _healConn:Disconnect(); _healConn=nil; return
        end
        ...
        task.spawn(function()
            ...
            if hpPct<Config.AutoHeal.HPThreshold then
                local r=Invoke(R.UseHeal)
                if r then ShowToast("Healed!",true) end
            end
...
local function StartAutoSell()
    ...
    _sellConn=RunService.Heartbeat:Connect(function(dt)
        if not Config.AutoSell.Enabled then
            _sellConn:Disconnect(); _sellConn=nil; return
        end
        t=t+dt; if t<Config.AutoSell.Interval then return end; t=0
        task.spawn(function()
            local r=Invoke(R.SellAll)
            if r then ShowToast("Sold all loot!",true) end
        end)
    end)
end
...
Core:Add(LocalPlayer.CharacterAdded:Connect(function()
    ...
    if Config.AutoAttack.Enabled then StartAutoAttack() end
end))
...
local function CreateESP(player)
    ...
    local c={}
    local ec=Color3.fromRGB(255,50,50)
    local function nL(t)
...
Core:Add(RunService.RenderStepped:Connect(function()
    for player,c in pairs(ESPCache) do
        ...
        if Config.ESP.ShowName then
            c.Text.Text=player.Name; c.Text.Position=Vector2.new(cx,tV.Y-16); c.Text.Visible=true
...
local THEME={
    ...
    topbar=Color3.fromRGB(16,8,24),
    card=Color3.fromRGB(16,10,24),
}

function UI:Build()
    local Screen=Instance.new("ScreenGui",SafeGUI)
    Screen.Name="SoloHunterUI"; Screen.ResetOnSpawn=false; Screen.DisplayOrder=999
    Core:Add(Screen)

    local Wrapper=Instance.new("Frame",Screen)
    Wrapper.Size=UDim2.new(0,255,0,460)
    ...
    WS.Color=THEME.primary; WS.Thickness=1.5
    ...
    TL.BackgroundTransparency=1; TL.Text="⚔️  SOLO HUNTER"
    ...
    Core:Add(PanicBtn.MouseButton1Click:Connect(function()
        Config.AutoAttack.Enabled=false; Config.AutoCollect.Enabled=false
        Config.AutoQuest.Enabled=false; Config.AutoHeal.Enabled=false
        Config.AutoSell.Enabled=false; Config.ESP.Enabled=false
    ...
    HideBtn.BackgroundColor3=Color3.fromRGB(40,20,60); HideBtn.Text="👁"
    HideBtn.TextColor3=Color3.fromRGB(200,150,255); HideBtn.Font=Enum.Font.GothamBold
    ...
    MinBtn.BackgroundColor3=Color3.fromRGB(30,15,45); MinBtn.Text="—"
    ...
    TabBar.BackgroundColor3=Color3.fromRGB(14,8,20); TabBar.BorderSizePixel=0
    ...
    Pill.BackgroundColor3=Color3.fromRGB(25,10,40); Pill.Text="⚔️ SOLO HUNTER"
    ...
    Core:Add(MinBtn.MouseButton1Click:Connect(function()
        ...
        Wrapper.Size=mini and UDim2.new(0,255,0,36) or UDim2.new(0,255,0,460)
...
function UI:AddTab(name)
    ...
    btn.BackgroundColor3=Color3.fromRGB(22,12,34)
    btn.Text=name; btn.TextColor3=Color3.fromRGB(140,80,200)
    ...
    local function activate()
        for _,t in ipairs(self._tabPages) do
            ...
            t.btn.BackgroundColor3=Color3.fromRGB(22,12,34)
            t.btn.TextColor3=Color3.fromRGB(140,80,200)
        end
        page.Visible=true
...
function UI:Toggle(parent,label,callback,col)
    ...
    lbl.TextColor3=Color3.fromRGB(210,185,240)
    ...
    pill.BackgroundColor3=Color3.fromRGB(35,20,50)
    pill.Text=""; pill.BorderSizePixel=0
    Instance.new("UICorner",pill).CornerRadius=UDim.new(1,0)
    local knob=Instance.new("Frame",pill)
    knob.Size=UDim2.new(0,11,0,11); knob.Position=UDim2.new(0,2,0.5,-5)
    knob.BackgroundColor3=Color3.fromRGB(120,80,160); knob.BorderSizePixel=0
    ...
    Core:Add(pill.MouseButton1Click:Connect(function()
        ...
        if state then
            ...
            knob.BackgroundColor3=Color3.fromRGB(120,80,160)
        end
...
function UI:MakeSlider(parent,labelText,initVal,minVal,maxVal,onChange)
    ...
    fl.TextColor3=Color3.fromRGB(190,160,220); fl.Font=Enum.Font.GothamSemibold
    ...
    tr.BackgroundColor3=Color3.fromRGB(35,20,50); tr.BorderSizePixel=0
...
function UI:ActionBtn(parent,label,col,cb)
    ...
    btn.Size=UDim2.new(1,0,0,28); btn.BackgroundColor3=col or THEME.primary
...
-- ── TAB: ATTACK ───────────────────────────────────────────────────────────
do
    local p,r=UI:AddTab("Atk")

    -- Info dari hasil spy
    local ic=Instance.new("Frame",p)
    ic.Size=UDim2.new(1,0,0,44); ic.BackgroundColor3=Color3.fromRGB(10,6,18)
    ic.BorderSizePixel=0
    Instance.new("UICorner",ic).CornerRadius=UDim.new(0,6)
    Instance.new("UIStroke",ic).Color=Color3.fromRGB(80,40,160)
    local il=Instance.new("TextLabel",ic)
    il.Size=UDim2.new(1,-10,1,-6); il.Position=UDim2.new(0,5,0,3)
    il.BackgroundTransparency=1
    il.Text="⚔️ UseWeapon(0,{},1.0) dari spy\n⚡ CancelAttacks skip animasi = DPS brutal"
    il.TextColor3=Color3.fromRGB(180,140,255); il.Font=Enum.Font.Gotham
    il.TextSize=10; il.TextXAlignment=Enum.TextXAlignment.Left; il.TextWrapped=true

    UI:Section(p,"AUTO ATTACK")
    UI:Toggle(p,"⚔️ Auto Attack (Brutal)",function(v)
        Config.AutoAttack.Enabled=v
        if v then StartAutoAttack() end
    end,Color3.fromRGB(220,50,255))

    UI:Toggle(p,"⚡ Skip Animasi (CancelAttacks)",function(v)
        Config.AutoAttack.SkipAnim=v
    end,Color3.fromRGB(180,80,255))

    UI:MakeSlider(p,"Min Delay (ms)",math.floor(Config.AutoAttack.MinDelay*1000),
        50,500,function(v) Config.AutoAttack.MinDelay=v/1000 end)

    UI:MakeSlider(p,"Max Delay (ms)",math.floor(Config.AutoAttack.MaxDelay*1000),
        100,800,function(v) Config.AutoAttack.MaxDelay=v/1000 end)

    UI:MakeSlider(p,"Range (stud)",Config.AutoAttack.Range,
        10,500,function(v) Config.AutoAttack.Range=v end)

    -- Stats realtime
    local statsCard=Instance.new("Frame",p)
    statsCard.Size=UDim2.new(1,0,0,28); statsCard.BackgroundColor3=Color3.fromRGB(10,6,18)
    statsCard.BorderSizePixel=0
    Instance.new("UICorner",statsCard).CornerRadius=UDim.new(0,6)
    Instance.new("UIStroke",statsCard).Color=Color3.fromRGB(80,40,160)
    local statsLbl=Instance.new("TextLabel",statsCard)
    statsLbl.Size=UDim2.new(1,-10,1,-6); statsLbl.Position=UDim2.new(0,5,0,3)
    statsLbl.BackgroundTransparency=1; statsLbl.Text="⚔️ Attack count: 0"
    statsLbl.TextColor3=Color3.fromRGB(180,140,255); statsLbl.Font=Enum.Font.GothamBold
    statsLbl.TextSize=11; statsLbl.TextXAlignment=Enum.TextXAlignment.Left
    Core:Add(RunService.Heartbeat:Connect(function()
        pcall(function()
            local target,dist=GetNearestMonster(Config.AutoAttack.Range)
            statsLbl.Text=string.format("⚔️ Count: %d | Target: %s",
                _atkCount,
                target and string.format("%s (%.0fm)",target.Name,dist or 0) or "none")
        end)
    end))
...
-- ── TAB: FARM ─────────────────────────────────────────────────────────────
do
    local p,r=UI:AddTab("Farm")

    UI:Section(p,"AUTO COLLECT")
    UI:Toggle(p,"Auto Collect Drop",function(v)
        Config.AutoCollect.Enabled=v; if v then StartAutoCollect() end
    end,Color3.fromRGB(255,180,50))
    UI:MakeSlider(p,"Collect Range",Config.AutoCollect.Range,
        50,500,function(v) Config.AutoCollect.Range=v end)

    UI:Section(p,"AUTO QUEST")
    UI:Toggle(p,"Auto Quest (Request+TurnIn)",function(v)
        Config.AutoQuest.Enabled=v; if v then StartAutoQuest() end
    end,Color3.fromRGB(80,200,255))

    UI:Section(p,"AUTO HEAL")
    UI:Toggle(p,"Auto Heal + Mana",function(v)
        Config.AutoHeal.Enabled=v; if v then StartAutoHeal() end
    end,Color3.fromRGB(80,220,80))
    UI:MakeSlider(p,"HP Threshold (%)",Config.AutoHeal.HPThreshold,
        10,90,function(v) Config.AutoHeal.HPThreshold=v end)

    UI:Section(p,"AUTO SELL")
    UI:Toggle(p,"Auto Sell Loot",function(v)
        Config.AutoSell.Enabled=v; if v then StartAutoSell() end
    end,Color3.fromRGB(255,200,50))
    UI:ActionBtn(p,"💰 Jual Sekarang",Color3.fromRGB(140,90,10),function()
        local r2=Invoke(R.SellAll)
        ShowToast(r2 and "Sold!" or "Gagal",not not r2)
    end)

    -- Quest counter
    local qCard=Instance.new("Frame",p)
    qCard.Size=UDim2.new(1,0,0,26); qCard.BackgroundColor3=Color3.fromRGB(10,6,18)
    qCard.BorderSizePixel=0
    Instance.new("UICorner",qCard).CornerRadius=UDim.new(0,6)
    local qLbl=Instance.new("TextLabel",qCard)
    qLbl.Size=UDim2.new(1,-10,1,-6); qLbl.Position=UDim2.new(0,5,0,3)
    qLbl.BackgroundTransparency=1; qLbl.Text="📋 Quest completed: 0"
    qLbl.TextColor3=Color3.fromRGB(100,200,255); qLbl.Font=Enum.Font.GothamBold
    qLbl.TextSize=11; qLbl.TextXAlignment=Enum.TextXAlignment.Left
    Core:Add(RunService.Heartbeat:Connect(function()
        pcall(function() qLbl.Text="📋 Quest completed: ".._questCount end)
    end))
...
do
    ...
    UI:Toggle(p,"Speed Hack",function(v)
        Config.Speed.Enabled=v; if v then StartSpeed() end
    end,THEME.primary)
    UI:MakeSlider(p,"Speed Value",Config.Speed.Value,
        16,200,function(v)
...
do
    ...
    sCard.Size=UDim2.new(1,0,0,60); sCard.BackgroundColor3=Color3.fromRGB(10,6,18)
    sCard.BorderSizePixel=0
    Instance.new("UICorner",sCard).CornerRadius=UDim.new(0,6)
    Instance.new("UIStroke",sCard).Color=Color3.fromRGB(80,40,160)
    ...
    sL.Text=string.format(
        "✅ Remote loaded: %d\n⚔️ UseWeapon: %s\n💊 Heal: %s | 📋 Quest: %s",
        loaded,
        R.UseWeapon and "OK" or "❌",
        R.UseHeal and "OK" or "❌",
        R.RequestQuest and "OK" or "❌"
    )
    sL.TextColor3=Color3.fromRGB(180,140,255); sL.Font=Enum.Font.Gotham
    sL.TextSize=10; sL.TextXAlignment=Enum.TextXAlignment.Left; sL.TextWrapped=true

    UI:Section(p,"TEMUAN SPY")
    local spyCard=Instance.new("Frame",p)
    spyCard.Size=UDim2.new(1,0,0,70); spyCard.BackgroundColor3=Color3.fromRGB(10,6,18)
    spyCard.BorderSizePixel=0
    Instance.new("UICorner",spyCard).CornerRadius=UDim.new(0,6)
    Instance.new("UIStroke",spyCard).Color=Color3.fromRGB(60,30,100)
    local spyL=Instance.new("TextLabel",spyCard)
    spyL.Size=UDim2.new(1,-14,1,-8); spyL.Position=UDim2.new(0,7,0,4)
    spyL.BackgroundTransparency=1
    spyL.Text="UseWeapon(0, {}, [3])\n[3]=timing: 0.58~1.0 alami\n[3]=1.0 selalu = full charge\nDamage: server-authoritative\n[3]=0.0 = tidak keluar"
    spyL.TextColor3=Color3.fromRGB(150,120,200); spyL.Font=Enum.Font.Code
    spyL.TextSize=9; spyL.TextXAlignment=Enum.TextXAlignment.Left; spyL.TextWrapped=true
...
print("⚔️ Auto Attack: UseWeapon(0,{},1.0) + CancelAttacks")
print("🌾 Auto Collect + Quest + Heal + Sell")
print("👁 ESP + Speed")
