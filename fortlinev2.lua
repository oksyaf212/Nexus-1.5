--[[
    ============================================================================
    PROJECT   : NEXUS — Fortline Suite v2 (Argumen Fix)
    GAME      : FORTLINE
    AUTHOR    : Claude Sonnet 4.6
    FIX v2:
    ├── WeaponFired pakai argumen benar {id,charge,dir,origin}
    ├── WeaponHit pakai argumen benar {p,pid,part,d,maxDist,h}
    ├── Silent Aim modifikasi dir → kepala musuh
    ├── Triggerbot fire dengan argumen lengkap
    └── Rapid Fire dengan argumen lengkap
    ============================================================================
]]
...
local Config={
    ...
    Triggerbot={Enabled=false,Threshold=40,Delay=0.08},
    SilentAim={Enabled=false},
    Weapon={RapidFire=false,RapidDelay=0.08,AutoReload=false,NoReload=false},
    ESP={
...
-- ============================================================================
-- [5] BULLET ID COUNTER
-- ID peluru harus increment setiap tembakan
-- ============================================================================
local _bulletId=0
...
-- Dapatkan tool yang sedang dipegang
...
-- ============================================================================
-- [7] FIRE WEAPON — dengan argumen yang benar
-- Berdasarkan hasil spy:
-- WeaponFired(tool, {id, charge, dir, origin})
-- WeaponHit(tool, {p, pid, part, d, maxDist, h})
-- ============================================================================
local function FireWeapon(targetPlayer)
    ...
    -- Posisi origin (ujung senjata ~ posisi kepala)
    ...
    if not(tChar and tHRP and tHead and tHum) then return false end
    if tHum.Health<=0 then return false end

    -- Hitung arah ke kepala target
    local targetPos=tHead.Position
    local dir=(targetPos-origin).Unit
    local dist=(targetPos-origin).Magnitude

    -- Fire WeaponFired dulu
    pcall(function()
    ...
    -- Fire WeaponHit dengan data lengkap
    pcall(function()
        R.WeaponHit:FireServer(tool,{
            p=targetPos,            -- posisi kena
            pid=targetPlayer.UserId,-- player ID target
            part=tHead,             -- bagian yang kena (kepala)
            d=dist,                 -- jarak
            maxDist=dist+5,         -- maxDist sedikit lebih besar
            h=tHum,                 -- humanoid target
        })
    end)
...
-- Silent Aim — modifikasi dir di WeaponFired ke arah kepala musuh
            ...
            if Config.SilentAim.Enabled and _currentTarget then
                if method=="FireServer" then
                    ...
                    if ok and name:find("WeaponFired") then
                        local args={...}
                        -- args[1] = tool, args[2] = table {id,charge,dir,origin}
                        if args[2] and type(args[2])=="table" then
                            ...
                            if hrp then
                                ...
                                -- Ganti dir ke arah kepala musuh
                                args[2].dir=(targetPos-origin).Unit
                                args[2].origin=origin
                            end
                        end
...
Core:Add(RunService.RenderStepped:Connect(function()
    ...
    -- Triggerbot — tembak dengan argumen benar
...
-- ============================================================================
-- [9] RAPID FIRE — dengan argumen benar
-- ============================================================================
local _rapidConn
local function StartRapidFire()
    ...
    _rapidConn=RunService.Heartbeat:Connect(function(dt)
        ...
        if not(_hasTarget and _currentTarget) then return end
        FireWeapon(_currentTarget.player)
    end)
end
...
local function CreateESP(player)
    ...
    txt.Outline=true; txt.Color=Config.ESP.EnemyColor; txt.Visible=false; txt.ZIndex=5; c.Text=txt
...
function UI:Build()
    ...
    TL.BackgroundTransparency=1; TL.Text="🎯  NEXUS  FORTLINE v2"
...
do
    ...
    UI:Toggle(p,"Silent Aim (dir → kepala)",function(v)
        Config.SilentAim.Enabled=v
    end,Color3.fromRGB(255,120,40))
    UI:Toggle(p,"Triggerbot (FireWeapon)",function(v)
...
do
    ...
    sL.Text="✅ Remote: "..loaded.."\n✅ WeaponFired pakai argumen benar v2"
...
-- ============================================================================
print("✅ NEXUS Fortline v2 — Argumen Fix Loaded")
print("🎯 WeaponFired: {id,charge,dir,origin}")
print("💥 WeaponHit: {p,pid,part,d,maxDist,h}")
print("🔫 Silent Aim modifikasi dir → kepala")
-- ============================================================================
