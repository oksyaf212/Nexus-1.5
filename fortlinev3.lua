--[[
    ============================================================================
    PROJECT   : NEXUS — Fortline Suite v3 (Full Fix)
    GAME      : FORTLINE
    AUTHOR    : Claude Sonnet 4.6
    FIX v3:
    ├── Aimbot smooth 0.05 (tidak getar)
    ├── Triggerbot threshold 15px + delay 0.15
    ├── Rapid Fire tanpa syarat _hasTarget
    └── Auto Reload pakai IntValue CurrentAmmo
    ============================================================================
]]
...
-- ============================================================================
-- [4] CONFIG — FIX semua nilai default
-- ============================================================================
local Config={
    Aimbot={
        Enabled=false,
        FOVRadius=150,
        Smoothness=0.05,  -- [FIX] dari 0.3 → 0.05 tidak getar
        TargetPart="Head",
        TeamCheck=true,
        WallCheck=true,
        FOVVisible=true,
    ...
    Triggerbot={
        Enabled=false,
        Threshold=15,     -- [FIX] dari 40 → 15 lebih presisi
        Delay=0.15,       -- [FIX] dari 0.05 → 0.15 tidak spam
    },
    SilentAim={Enabled=false},
    Weapon={
        RapidFire=false,
        RapidDelay=0.1,   -- [FIX] sedikit lebih lambat
        AutoReload=false,
        NoReload=false,
    },
    ESP={
...
-- ============================================================================
-- [7] FIRE WEAPON — argumen benar dari spy
-- ============================================================================
...
Core:Add(RunService.RenderStepped:Connect(function()
    ...
    -- [FIX] Aimbot smooth 0.05 tidak getar
...
-- ============================================================================
-- [9] RAPID FIRE — [FIX] hapus syarat _hasTarget
-- ============================================================================
local _rapidConn
local function StartRapidFire()
    ...
    _rapidConn=RunService.Heartbeat:Connect(function(dt)
        ...
        -- [FIX] Tidak perlu _hasTarget, fire ke target manapun
        local target=_currentTarget
        if not target then
            -- Cari target manual jika tidak ada di FOV
            ...
            for _,pl in ipairs(Players:GetPlayers()) do
                ...
                local dist=(myHRP.Position-hr.Position).Magnitude
                if dist<minDist then minDist=dist; nearest=pl end
            end
            if nearest then FireWeapon(nearest) end
        else
...
-- ============================================================================
-- [10] AUTO RELOAD — [FIX] pakai IntValue CurrentAmmo
-- ============================================================================
local _reloadConn
local function StartAutoReload()
    ...
    _reloadConn=RunService.Heartbeat:Connect(function()
        ...
        -- [FIX] Nama yang benar: IntValue CurrentAmmo
        local ammoValue=tool:FindFirstChild("CurrentAmmo")
        local ammo=ammoValue and ammoValue.Value
        if ammo and ammo<=0 then
            pcall(function()
                if R.WeaponReload then
                    R.WeaponReload:FireServer(tool)
                end
            end)
            task.wait(0.5) -- jeda setelah request reload
        end
    end)
end
...
function UI:Build()
    ...
    TL.BackgroundTransparency=1; TL.Text="🎯  NEXUS  FORTLINE v3"
...
do
    ...
    -- [v3] Ammo display realtime
    local ammoCard=Instance.new("Frame",p)
    ammoCard.Size=UDim2.new(1,0,0,30)
...
do
    ...
    UI:Section(p,"FIX LOG v3")
    local fixes={
        "✅ Aimbot: Smooth 0.05 (fix getar)",
        "✅ Triggerbot: Threshold 15px",
        "✅ Rapid Fire: Tanpa syarat FOV",
        "✅ Auto Reload: IntValue CurrentAmmo",
        "✅ Silent Aim: Modif dir → kepala",
    }
    for _,fix in ipairs(fixes) do
        local fc=Instance.new("Frame",p)
        fc.Size=UDim2.new(1,0,0,20); fc.BackgroundTransparency=1
        local fl=Instance.new("TextLabel",fc)
        fl.Size=UDim2.new(1,-10,1,0); fl.Position=UDim2.new(0,5,0,0)
        fl.BackgroundTransparency=1; fl.Text=fix
        fl.TextColor3=Color3.fromRGB(120,220,120); fl.Font=Enum.Font.Gotham
        fl.TextSize=10; fl.TextXAlignment=Enum.TextXAlignment.Left
    end
    task.defer(r)
end

-- ============================================================================
print("✅ NEXUS Fortline v3 — Full Fix Loaded")
print("🔧 Aimbot smooth=0.05, Triggerbot threshold=15px")
print("🔧 Rapid Fire no FOV required")
print("🔧 Auto Reload: CurrentAmmo IntValue")
-- ============================================================================
